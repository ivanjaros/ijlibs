...
import (
	...
	"bitbucket.org/ivanjaros/seeme/libs/rtmp/format/flv"
...
func Serve(l net.Listener, handler func(*Conn)) (err error) {
	if Debug {
		fmt.Println("rtmp: server: listening on", l.Addr().String())
	}
	...
	for {
		...
		go func() {
			rConn := NewConn(conn)
			rConn.isserver = true
			err := rConn.prepare(stageCommandDone, 0)
			if err != nil {
				return
			}
			handler(rConn)
